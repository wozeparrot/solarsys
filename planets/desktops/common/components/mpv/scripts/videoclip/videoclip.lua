...
local config = {
    ...
    audio_folder_path = string.format("%s/videos/", os.getenv("HOME") or os.getenv("USERPROFILE")),
...
main_menu.keybindings = {
    ...
    },
    {
        key = "o",
        fn = function()
            mp.commandv("run", "xdg-open", "https://streamable.com/")
        end,
    },
    {
        key = "ESC",
...
function pref_menu:cycle_audio_bitrates()
    self.audio_bitrates.selected = self.audio_bitrates.selected + 1 > #self.audio_bitrates and 1
        or self.audio_bitrates.selected + 1
